...
mdfile='sysA'                        # sysA, sysB, evbless
topol='topol_000.top'                # topol_000.top, topol_050.top, evbless.top
structure='equil_ions.gro'
...
if [[ ${mdfile} != 'evbless' ]]; then
  ...
  for step in ${steps[@]}; do
    ...
    if ${GMX_BIN} mdrun -s ${mdfile}.tpr -rerun ${step}.trr -table table.xvg -tableb table_b0.xvg -e ${step}_${mdfile}.edr -g ${step}_${mdfile}.log -o ${step}_${mdfile}.trr -dhdl ${step}_${mdfile}.xvg -pin on -ntmpi 1 -ntomp 4
  ...
  for step in ${steps[@]}; do
    echo -n "Time: $(date)     " >> ${mdfile}.txt
    echo -n "Running the step: ${step}    --    " >> ${mdfile}.txt
    if ${GMX_BIN} mdrun -s ${mdfile}.tpr -rerun ${step}.trr -e ${step}_${mdfile}.edr -g ${step}_${mdfile}.log -o ${step}_${mdfile}.trr -dhdl ${step}_${mdfile}.xvg -pin on -ntmpi 1 -ntomp 4
      then echo "OK" >> ${mdfile}.txt
      rsync -ah --update $RUNDIR/ $SLURM_SUBMIT_DIR/ --exclude="*tmp"
    else
      echo "FAILED" >> ${mdfile}.txt
