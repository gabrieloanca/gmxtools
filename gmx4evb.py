...
def read_qm(qm, rs, ps):
    ...
    for i,l in enumerate(data):
        ...
        try:
            ...
            print(f"\nTere is an error in {qm} file at line {i}.")
            sys.exit()
...
def pairs_list(rs, ps, rs_files, ps_files, qpdb, q1, q2):
    ...
    # kepp pairs that belong only to one of the reactants states
    def sift(a, b):
        holder = []
        for p1 in a:
            chk = True
    ...
    def classify(pairs_x2y, pairs, bonds, angles, state):
        ...
        for p in pairs:
            ...
            chk_b = False # checks if pair in bonds
            chk_a = False # checks if pair in angles
            ...
            for b in bonds:
                if (a[0][0] in b[:2]) and (a[0][2] in b[:2]):
                    if b[2] == 3: # if Morse, the list is larger then if it is harmonic
                        ...
                        if a[1] == 35 and b[6]:    # b[6] checks for k_Morse(PS)
                            angles_solo.pop(i)
                            chk = True
                            break
                        elif a[1] == 53 and b[4]:  # b[4] checks for k_Morse(RS)
...
def soft_core(at_ad, bonds_solo, angles_solo, soft_at, soft_pairs):
    ...
    try:
        for p in soft_pairs:
            ...
            if not (betaA == betaB):
                print(f'Table numbers cannot change from A to B. Beta = {betaA} will be used')
    ...
    for p in at_ad:
        ...
        if (p1 in soft_at.keys()) and (p2 in soft_at.keys()):
            ...
            if not (((p1, p2) in init_pairs) or ((p2, p1) in init_pairs)):
                soft.append([p1, p2, func, i, A, i, A, beta])
...
# forming bonds of type 3 (morse) in PS state which generates exclusionns
...
def pairs_nb(at_ad, soft, nb55, bonds_solo, angles_solo, pairs_x2y, charges, rs_vdw, ps_vdw, l): # l = lambda = 0..1
    ...
    # restore the excluded pairs of atoms due to the forming/breaking bonds which generate exclusions
    for p in nb55:
        at1, at2 = p
        tick = True
        if ([at1, at2] in at_ad) or ([at2, at1] in at_ad):
            ...
            if tick:
                if l == 0:
                    ...
                    print("If you want to avoid adding van der Waals interaction, then add a null soft-core.")
                    print()
            else:
                q1A = float(charges[at1][0])
                q2A = float(charges[at2][0])
...
if __name__ == "__main__":
    ...
    # Show reminders for printing tables    
    for i, j in enumerate(betas):
        print(f"Don't forget to built table_b{i}.xvg with beta = {j:6.2f}")
